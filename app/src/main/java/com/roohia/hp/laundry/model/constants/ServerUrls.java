package com.roohia.hp.laundry.model.constants;

public class ServerUrls {
    public static final String STAGING_BASE_URL = "http://testingUrl/Laundra";
    public static final String LIVE_BASE_URL = "http://hslogiq.com/laundry/API";
    public static final String BASE_URL = LIVE_BASE_URL;
    public static final String LOGIN_URL = "/LaundryLoginApiCallClass.php";
    public static final String SIGNUP_URL = "/LaundrySignupApiCallClass.php";
    public static final String UPDATE_USER_URL = "/LaundryUpdateProfileApiCallClass.php";



}
